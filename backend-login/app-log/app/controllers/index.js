...
var self = (module.exports = {
    test: (req, res) => {
        return res
            .status(200)
            .json({ status: _CONST.SUCCESS, elements: 'Hello Anh Em' });
    },
    home: async (req, res) => {
        ...
        if (req.method === 'POST') {
            ...
            if (q.TYPE === 'RE') {
                ...
                if (rsLogin) {
                    ...
                    if (rsUser) {
                        return res.redirect(
                            'http://localhost:8080/login?step=login',
                        );
                    }
                }

                return res.redirect(
                    'http://localhost:8080/login?step=error_login',
                );
            }
            ...
            if (q.TYPE === 'LO') {
                ...
                if (objLogin) {
                    ...
                    return res.redirect(
                        'http://localhost:8080/profile?step=login_success',
                    );
                }
